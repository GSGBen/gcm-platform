...
module modBastionPublicIp 'publicIp.bicep' = if (parAzBastionEnabled) {
...
module modGatewayPublicIp 'publicIp.bicep' = [for (gateway, i) in varGwConfig: if ((gateway.name != 'noconfigVpn') && (gateway.name != 'noconfigEr')) {
...
module modPrivateDnsZones 'privateDnsZones.bicep' = if (parPrivateDnsZonesEnabled) {
